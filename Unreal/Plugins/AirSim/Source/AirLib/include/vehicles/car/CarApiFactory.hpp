...
#include "vehicles/car/firmwares/chaoscar/ChaosCarApi.hpp"
                                                     ...
                                                     const Kinematics::State& state, const Environment& environment)
        {
            if (vehicle_setting->vehicle_type == AirSimSettings::kVehicleTypeArduRover) {
                return std::unique_ptr<CarApiBase>(new ArduRoverApi(vehicle_setting, sensor_factory, state, environment));
                     ...
                     vehicle_setting->vehicle_type == AirSimSettings::kVehicleTypeChaosCar) {
                return std::unique_ptr<CarApiBase>(new ChaosCarApi(vehicle_setting, sensor_factory, state, environment));
            }
            else
