...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kVehicleTypeChaosCar = "chaoscar";
    ...
    private:
        ...
        void loadCoreSimModeSettings(const Settings& settings_json, std::function<std::string(void)> simmode_getter)
        {
            ...
            if (physics_engine_name == "") {
                if (simmode_name == kSimModeTypeMultirotor)
                    ...
                    physics_engine_name = "Chaos"; //this value is only informational for now
            }
        }
                                         ...
                                         const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
        {
            ...
            if (simmode_name == kSimModeTypeMultirotor) {
                ...
                // create Chaos as default car vehicle
                auto chaos_car_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("ChaosCar", kVehicleTypeChaosCar));
                chaos_car_setting->sensors = sensor_defaults;
                vehicles[chaos_car_setting->vehicle_name] = std::move(chaos_car_setting);
            }
            else if (simmode_name == kSimModeTypeComputerVision) {
